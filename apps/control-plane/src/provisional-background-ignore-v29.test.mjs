...
const authorizationPort = fs.readFileSync(new URL("./semantic-tool-authorization-port.ts", import.meta.url), "utf8");
...
test("v29 preserves one authoritative business tool per caller turn through the neutral authorization boundary", () => {
  assert.match(source, /publicRestaurantToolAuthorizationPortFor\(this\)\.decide\(event\)/);
  assert.match(authorizationPort, /malformedToolCorrectionRuntimeFor\(session\)\.preauthorize/);
  assert.match(authorizationPort, /authorizePublicRestaurantTool\(session, request\)/);
  ...
  assert.doesNotMatch(source, /authorizePublicRestaurantTool\(this/);
});
