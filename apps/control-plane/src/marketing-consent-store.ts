...
export type RecordMarketingOfferInput = {
  phone: string;
  callId: string;
...
export class SupabaseMarketingConsentStore {
  ...
  private headers(extra: Record<string, string> = {}): Record<string, string> {
    return { apikey: this.secretKey, Accept: "application/json", ...extra };
  }
  ...
  async getLatestStatus(tenantId: string, phoneValue: string): Promise<MarketingConsentStatus | null> {
    ...
    const query = new URLSearchParams({ select: "status", tenant_id: `eq.${tenant}`, phone: `eq.${phone}`, order: "created_at.desc", limit: "1" });
    const response = await fetch(`${this.baseUrl}/rest/v1/marketing_consents?${query.toString()}`, { method: "GET", headers: this.headers() });
    ...
    if (parsed.length !== 1 || !parsed[0] || typeof parsed[0] !== "object") throw new Error("Supabase marketing_consents returned invalid read payload");
    return parseStatus((parsed[0] as { status?: unknown }).status);
  }

  async getLatestOfferAt(tenantId: string, phoneValue: string): Promise<string | null> {
    const tenant = assertTenantId(tenantId);
    const phone = normalizeE164(phoneValue);
    const query = new URLSearchParams({ select: "offered_at", tenant_id: `eq.${tenant}`, phone: `eq.${phone}`, order: "offered_at.desc", limit: "1" });
    const response = await fetch(`${this.baseUrl}/rest/v1/marketing_consent_offers?${query.toString()}`, { method: "GET", headers: this.headers() });
    const raw = await response.text();
    if (!response.ok) throw new Error(`Supabase marketing_consent_offers read failed with HTTP ${response.status}`);
    let parsed: unknown;
    try { parsed = JSON.parse(raw); } catch { throw new Error("Supabase marketing_consent_offers returned invalid JSON"); }
    if (!Array.isArray(parsed)) throw new Error("Supabase marketing_consent_offers returned invalid read payload");
    if (parsed.length === 0) return null;
    const offeredAt = (parsed[0] as { offered_at?: unknown })?.offered_at;
    if (typeof offeredAt !== "string" || !Number.isFinite(Date.parse(offeredAt))) throw new Error("Supabase marketing_consent_offers returned invalid offered_at");
    return offeredAt;
  }

  async recordOffer(tenantId: string, input: RecordMarketingOfferInput): Promise<void> {
    const tenant = assertTenantId(tenantId);
    ...
    const response = await fetch(`${this.baseUrl}/rest/v1/marketing_consent_offers`, {
      method: "POST",
      headers: this.headers({ "Content-Type": "application/json", Prefer: "return=minimal" }),
      body: JSON.stringify({ tenant_id: tenant, phone, call_id: callId, policy_version: policyVersion }),
    });
    if (!response.ok) throw new Error(`Supabase marketing_consent_offers insert failed with HTTP ${response.status}`);
  }
  ...
  async record(tenantId: string, input: RecordMarketingConsentInput): Promise<MarketingConsentEvent> {
    ...
    if (input.verificationMethod === "CALLER_ID_MATCH" && callerPhone !== phone) throw new Error("CALLER_ID_MATCH requires caller phone to equal marketing phone");
    ...
    const response = await fetch(`${this.baseUrl}/rest/v1/marketing_consents`, {
      ...
      body: JSON.stringify({
        tenant_id: tenant, phone, channel: "phone", status, consent_text_version: version,
        consented_at: input.action === "GRANT" ? now : null,
        verified_at: input.action === "GRANT" ? now : null,
        revoked_at: input.action === "REVOKE" ? now : null,
        source: "voice", verification_method: input.verificationMethod, caller_phone: callerPhone, call_id: callId,
      }),
    });
    const raw = await response.text();
