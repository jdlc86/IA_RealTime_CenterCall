...
export const RESERVATION_SEARCH_LOCAL_TIME_INSTRUCTION = "Presenta como máximo tres opciones usando starts_at_spoken, que incluye día de la semana, fecha y hora de Madrid. Nunca digas solamente «ese día» ni verbalices o reutilices starts_at_utc como si fuese hora local. Estas opciones ya están filtradas por horario comercial; no reserves hasta que el cliente elija una y pase por restaurant_reservation_create.";
...
export function formatMadridReservationSpeech(iso: string): string | null {
  const date = new Date(iso);
  if (!Number.isFinite(date.getTime())) return null;
  ...
  if (!weekday || !day || !month || !hour || !minute) return null;
...
export function localizeReservationSearchToolResult(request: RealtimeToolResultRequest): RealtimeToolResultRequest {
  ...
  const options = payload.options.map((value) => {
    ...
    return {
      ...
      starts_at_spoken: startsAt ? formatMadridReservationSpeech(startsAt) : null,
      timezone: RESERVATION_TIMEZONE,
    };
  });
