...
export { CallSession } from "./call-session-v8";
...
async function verifyTelnyxSignature(rawBody: string, request: Request, publicKeyValue: string): Promise<boolean> {
...
async function transferToRealtime(
  ...
  if (!response.ok) {
    ...
    console.error(JSON.stringify({
      level: "error",
      event: "telnyx_transfer_with_caller_failed",
...
async function handleTelnyxWebhook(request: Request, env: WorkerEnv, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
    event = JSON.parse(rawBody) as TelnyxVoiceEvent;
  } catch {
  ...
  const callControlId = payload.call_control_id?.trim();
  ...
  let resolution: TenantResolutionV1 | null;
  try {
