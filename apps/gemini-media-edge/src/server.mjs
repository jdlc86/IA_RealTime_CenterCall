...
const runtime = createGeminiMediaEdgeRuntime({ geminiApiKey: process.env.GEMINI_API_KEY, verifyCredential, consumeCredentialOnce: (credentialId, notAfterEpochMs, nowEpochMs) => credentialConsumer.consume(credentialId, notAfterEpochMs, nowEpochMs), consumeBootstrapForClaims: (claims, nowEpochMs) => bootstrapRegistry.consumeForClaims(claims, nowEpochMs), bindControlSession: (claims, sink) => controlRegistry.bindCommandSink(claims, sink), isControlSessionActive: (claims) => controlRegistry.isActive(claims), emitControlEvent: (claims, event) => controlRegistry.emit(claims, event), observeDiagnostic: (diagnostic) => console.log(JSON.stringify({ event: "gemini_media_edge_diagnostic", ...diagnostic })), authoritativeTranscribe, synthesizeGovernedSpeech, callerVadConfig, model: process.env.GEMINI_LIVE_MODEL, maxBufferedBytes: process.env.MEDIA_EDGE_MAX_BUFFERED_BYTES ? Number(process.env.MEDIA_EDGE_MAX_BUFFERED_BYTES) : undefined });
const controlWss = new WebSocketServer({ noServer: true, perMessageDeflate: false, maxPayload: 256 * 1024 });
controlWss.on("connection", (socket, request) => { const url = new URL(request.url, "http://localhost"); const claims = { tenantId: required(url.searchParams.get("tenant_id"), "control tenant_id"), callControlId: required(url.searchParams.get("call_control_id"), "control call_control_id") }; let attachment; try { attachment = controlRegistry.attach(claims, (event) => { if (socket.readyState !== 1) return false; socket.send(JSON.stringify(event)); return true; }, () => socket.readyState === 1); } catch { socket.close(1008, "control session already attached"); return; } socket.on("message", (raw) => { try { Promise.resolve(controlRegistry.command(claims, JSON.parse(raw.toString("utf8")))).catch(() => { try { socket.close(1008, "invalid control command"); } catch {} }); } catch { socket.close(1008, "invalid control command"); } }); socket.on("close", attachment.detach); socket.on("error", attachment.detach); });

const server = http.createServer(async (request, response) => {
  if (request.url === "/ready" && request.method === "GET") { response.writeHead(200, { "content-type": "application/json; charset=utf-8" }); response.end(JSON.stringify({ ok: true, service: "gemini-media-edge", activeSessions: runtime.activeSessions(), controlSessions: controlRegistry.size() })); return; }
