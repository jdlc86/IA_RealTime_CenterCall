...
test("Fast canary pins and synchronizes shared secrets before deploying Cloud Run", async () => {
  const source = await workflow("gemini-fast-canary-deploy.yml");

  assert.match(source, /id: shared_secrets/);
  assert.match(source, /gcloud secrets versions describe "\$CONTROL_TOKEN_SECRET_VERSION"/);
  assert.match(source, /gcloud secrets versions describe "\$CREDENTIAL_SECRET_VERSION"/);
  ...
  assert.match(source, /MEDIA_EDGE_CONTROL_PLANE_TOKEN=gemini-media-edge-control-plane-token:\$\{\{ steps\.shared_secrets\.outputs\.control_version \}\}/);
  assert.match(source, /MEDIA_EDGE_CREDENTIAL_HMAC_SECRET=gemini-media-edge-credential-hmac-secret:\$\{\{ steps\.shared_secrets\.outputs\.credential_version \}\}/);
});
