...
export function canonicalFastBootstrap(value, nowEpochMs = Date.now()) {
  ...
  return Object.freeze({
    ...
    tenantId: required(value.tenantId, "Fast Gemini bootstrap tenantId", 256),
    callControlId: required(value.callControlId, "Fast Gemini bootstrap callControlId", 512),
    notAfterEpochMs,
...
export class InMemoryFastBootstrapRegistry {
  ...
  consumeForClaims(claims, nowEpochMs = Date.now()) {
    ...
    if (bootstrap.tenantId !== claims.tenantId || bootstrap.callControlId !== claims.callControlId || bootstrap.notAfterEpochMs !== claims.notAfterEpochMs) {
