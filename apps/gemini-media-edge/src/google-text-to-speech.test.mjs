...
function synthesizer(overrides = {}) {
  ...
  const synthesize = createGoogleTextToSpeechSynthesizer({
    ...
    fetcher: async (url, init) => {
      calls.push({ url, init });
      return new Response(JSON.stringify({ audioContent: Buffer.from([1, 2, 3, 4]).toString("base64") }), {
        status: 200,
...
test("governed TTS sends exact text and requests raw headerless 16 kHz PCM", async () => {
...
test("governed TTS rejects malformed PCM responses and enforces bounded audio", async () => {
  const odd = synthesizer({ fetcher: async () => new Response(JSON.stringify({ audioContent: Buffer.from([1, 2, 3]).toString("base64") }), { status: 200 }) }).synthesize;
  await assert.rejects(() => odd({ text: "Hola" }), /invalid PCM16 audio/);

  const tooLarge = synthesizer({
    maxAudioBytes: 4,
    fetcher: async () => new Response(JSON.stringify({ audioContent: Buffer.alloc(6).toString("base64") }), { status: 200 }),
  }).synthesize;
