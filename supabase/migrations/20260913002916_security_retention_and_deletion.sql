...
create index if not exists caller_security_events_retention_idx
  on public.caller_security_events (created_at, id);
create index if not exists caller_security_state_retention_idx
  on public.caller_security_state (
    (coalesce(last_seen_at, updated_at, created_at)), tenant_id, caller_key
  )
  where permanent_block = false and risk_score = 0;
create index if not exists human_handoff_events_retention_idx
  on public.human_handoff_events (updated_at, id)
  where status in (
    'TRANSFERRED', 'NO_ANSWER', 'BUSY', 'FAILED', 'CALLBACK_REQUIRED', 'TERMINATED'
  );
create index if not exists security_retention_runs_completed_idx
