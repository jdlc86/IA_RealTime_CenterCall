-- SEC-P1-04: bounded retention and deletion for Gemini Fast security data.
--
-- This maintenance path runs inside Postgres. It is deliberately disconnected
-- from admission, tool execution and the realtime audio path.
...
create table if not exists private.security_retention_runs (
  id bigint generated by default as identity primary key,
  started_at timestamptz not null,
  completed_at timestamptz not null,
  diagnostics_deleted integer not null default 0,
  call_attempts_deleted integer not null default 0,
  ordinary_security_events_deleted integer not null default 0,
  high_security_events_deleted integer not null default 0,
  administrative_events_deleted integer not null default 0,
  inactive_caller_states_deleted integer not null default 0,
  completed_handoffs_deleted integer not null default 0,
  retention_audit_rows_deleted integer not null default 0,
  permanent_blocks_due_review integer not null default 0,
  callbacks_due_review integer not null default 0,
  callbacks_overdue_review integer not null default 0,
  total_deleted integer not null,
  batch_size integer not null,
  max_rows integer not null,
  constraint security_retention_runs_nonnegative_chk check (
    diagnostics_deleted >= 0
    and call_attempts_deleted >= 0
    and ordinary_security_events_deleted >= 0
    and high_security_events_deleted >= 0
    and administrative_events_deleted >= 0
    and inactive_caller_states_deleted >= 0
    and completed_handoffs_deleted >= 0
    and retention_audit_rows_deleted >= 0
    and permanent_blocks_due_review >= 0
    and callbacks_due_review >= 0
    and callbacks_overdue_review >= 0
    and total_deleted >= 0
  ),
  constraint security_retention_runs_limits_chk check (
    batch_size between 1 and 1000
    and max_rows between 1 and 10000
    and total_deleted <= max_rows
  )
);

revoke all on table private.security_retention_runs
  from public, anon, authenticated, service_role;
revoke all on sequence private.security_retention_runs_id_seq
  from public, anon, authenticated, service_role;

comment on table private.security_retention_runs is
  'Aggregate SEC-P1-04 maintenance evidence. Contains counts and timestamps only; retained for 365 days.';

-- Each purge predicate starts with its retention timestamp so the maintenance
-- job can find old rows without scanning the live working set.
create index concurrently if not exists call_diagnostic_events_retention_idx
  on public.call_diagnostic_events (created_at, id);
create index concurrently if not exists caller_security_events_retention_idx
  on public.caller_security_events (created_at, id);
create index concurrently if not exists caller_security_state_retention_idx
  on public.caller_security_state (
    (coalesce(last_seen_at, updated_at, created_at)), tenant_id, caller_key
  )
  where permanent_block = false
    and risk_score = 0
    and security_strikes = 0
    and rate_limit_blocks = 0;
create index concurrently if not exists human_handoff_events_retention_idx
  on public.human_handoff_events (updated_at, id)
  where status in (
    'TRANSFERRED', 'NO_ANSWER', 'BUSY', 'FAILED', 'CALLBACK_REQUIRED', 'TERMINATED'
  );
create index concurrently if not exists security_retention_runs_completed_idx
  on private.security_retention_runs (completed_at, id);
...
comment on table public.call_diagnostic_events is
  'Server-only redacted technical call traces. Automatic bounded retention: 7 days.';
comment on table public.caller_security_events is
  'Pseudonymous caller-security evidence. CALL_ATTEMPT: 7 days; ordinary signals: 30 days; HIGH/CRITICAL: 90 days; administrative reset audit: 365 days.';
comment on table public.caller_security_state is
  'Current pseudonymous caller-security decision state. Inactive unblocked zero-risk state: 90 days. Permanent blocks require annual review and are never auto-deleted.';
comment on table public.human_handoff_events is
  'Restricted operational handoff audit. Completed or resolved handoffs: 30 days. Pending callbacks require review after 30 days and escalation after 90 days; they are never auto-deleted.';
